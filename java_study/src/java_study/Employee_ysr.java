...
public class Employee extends Person {
	...
	public static void main(String[] args) {
		...
		// 과제 5 추가
		Vector<Person> v=new Vector<Person>();	//Person을 저장할 벡터를 생성한 뒤, 5명의 employee 객체를 저장
		v.add(em1);
		v.add(em2);
		v.add(em3);
		v.add(em4);
		...
		/*
		Employee[] arr=new Employee[v.size()];	//배열 객체를 생성하고 메모리를 할당한다. -> v.get 써서 벡터 값을 반환하는 벡터 메소드. get/add/remove
		...
		*/
		...
		for(int i=0; i<arr.length; i++) {
			System.out.println(arr[i].getName()+", "+arr[i].getBirthDate()+", "+arr[i].gethomeAddress()+", "+v.get(i).getMonthlySalary()+", "+arr[i].getEmployeeID());
		}
		System.out.println("===================================");
		*/
